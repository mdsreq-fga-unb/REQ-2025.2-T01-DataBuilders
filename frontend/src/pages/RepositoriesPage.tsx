...
import { searchRepos, type RepoItem } from '../services/repos';
...
function RepositoriesPage() {
  ...
  const [remoteRepos, setRemoteRepos] = useState<RepoItem[]>([]);
  ...
  const filteredAndSortedRepositories = useMemo(() => {
    const toCards = (items: RepoItem[]) => items.map(i => ({
      title: i.name,
      description: i.description || '',
      language: i.language || 'N/A',
    ...
    let source = allRepositories;
    if (remoteRepos.length > 0) {
      source = toCards(remoteRepos);
    }
    let filtered = [...source];
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'relevancia':
        default:
          return (b.stars + b.dateValue.getTime() / 1000000) - (a.stars + a.dateValue.getTime() / 1000000);
      }
    });

    return filtered;
  }, [searchQuery, language, minStars, topic, updated, sortBy, hasReadme, hasRecentCommits, remoteRepos]);

  useEffect(() => {
    const shouldFetch = searchQuery.trim().length > 0 || language !== 'todas';
    if (!shouldFetch) {
      setRemoteRepos([]);
      return;
    }
    const sortMap: Record<string, 'stars' | 'updated' | undefined> = {
      'mais-stars': 'stars',
      'mais-recentes': 'updated',
    };
    setLoading(true);
    setApiError('');
    searchRepos({
      q: searchQuery.trim() || 'estrutura dados',
      language: language !== 'todas' ? language : undefined,
      sort: sortMap[sortBy],
      deep: true,
      per_page: 20,
    })
      .then(items => setRemoteRepos(items))
      .catch(err => setApiError(err?.message || 'Falha ao buscar repositórios'))
      .finally(() => setLoading(false));
  }, [searchQuery, language, sortBy]);
